...
fn disconnected_local_socket_flow_can_reconnect_with_ciphertext_only_delivery(
    ...
    let empty_result = adapter.run_conformance(b"");
    assert!(matches!(
        empty_result,
        Err(TransportError::Unavailable(message)) if message == "empty ciphertext"
    ));
...
fn plaintext_failure_does_not_prevent_later_reconnect() -> Result<(), TransportError> {
    ...
    let plaintext_result = adapter.run_conformance(b"prefix forbidden plaintext suffix");
    assert!(matches!(
        plaintext_result,
