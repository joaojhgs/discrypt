...
Scope:
- Strengthen the existing env-gated public MQTT + TURN WebRTC DataChannel test so it fails closed unless relay-only TURN policy, relay candidate evidence, DataChannel open, and bidirectional opaque text/control receipt are proven.
- Add a branch-scoped browser WebRTC coturn proof because the current Rust `webrtc` 0.20.0-alpha.1 Sans-I/O gatherer does not implement TURN relay candidate gathering.
...
Acceptance criteria:
- With `DISCRYPT_PUBLIC_TURN_E2E=1` and configured TURN credentials, the browser proof runs with `iceTransportPolicy: "relay"` and the Rust harness keeps its `WebRtcIceTransportPolicy::RelayOnly` contract for the future dependency-fixed cargo proof.
...
- The current Rust WebRTC dependency limitation must remain explicit; do not mark the Rust transport stack TURN-ready from the browser artifact alone.
...
- With credentials available for the browser proof: `DISCRYPT_PUBLIC_TURN_E2E=1 DISCRYPT_PUBLIC_TURN_ENDPOINT=<redacted> DISCRYPT_PUBLIC_TURN_USERNAME=<redacted> DISCRYPT_PUBLIC_TURN_CREDENTIAL=<redacted> node scripts/per30-browser-turn-proof.mjs`
- CI branch proof: `PER-30 configured TURN proof` in `.github/workflows/ci.yml` starts coturn, runs the browser proof with `DISCRYPT_PUBLIC_TURN_E2E=1`, validates the redacted artifact, and uploads `per30-configured-turn-proof-<run>-<attempt>`.
