...
- The new media payload transport gate `public_mqtt_signals_real_webrtc_media_frame_roundtrip` now exercises provider-signaled DataChannel routing with codec-like encrypted media payloads and return receipts as a dedicated, opt-in production gate on the transport/media boundary. This is not yet decoded remote audio transport; it proves encrypted-media-shaped payload carriage only.
