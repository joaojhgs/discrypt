...
- Adds an env-gated Tauri two-profile receipt proof for MQTT where Alice's serialized encrypted `TextMessageEnvelope` crosses the provider-signaled WebRTC DataChannel to the Bob-side transport peer, Bob's profile signs a `TextDeliveryReceipt`, that receipt returns over the same DataChannel as an opaque control frame, and Alice only applies `peer_receipt` after signature verification. This is closer to production receipt delivery than the local-only signer test, but remains a same-process harness, not a persistent installed-app session/receiver loop.
...
- Latest live Tauri MQTT receipt proof passed with `DISCRYPT_DESKTOP_PUBLIC_MQTT_RECEIPT_E2E=1` against `mqtts://broker.emqx.io:8883`. It sends Alice's stored encrypted `TextMessageEnvelope` as the request frame, returns Bob's signed `TextDeliveryReceipt` as the response frame, verifies both DataChannel directions, then applies `peer_receipt` through the real receipt verifier. This proves the receipt payload can cross the provider-signaled WebRTC DataChannel, but does not yet provide persistent runtime sessions or a receiver-driven event loop.
...
- [ ] Complete IPFS/libp2p production hardening:
  - public/default bootstrap peer policy and local resource limits are now configured and tested: versioned discovery-only `bootstrap.libp2p.io` seed policy, max 16 bootstrap endpoints, duplicate rejection, 64 KiB envelope/transmit limit, bounded command queue, strict gossipsub validation, flood-publish disabled, and bounded mesh/history/duplicate-cache settings,
  - typed IPFS health now covers `topic_mesh_unavailable` as `provider_unhealthy` and oversized envelopes as `provider_message_too_large`, while invalid/duplicate/overflow profile rejection remains policy-level; remaining typed-health gaps are bootstrap connection failures, duplicate storms, and provider-unhealthy swarm events,
...
- [ ] Dependency/security audit for `rumqttc` and any Nostr/IPFS/libp2p dependencies. Latest `cargo audit` is documented in `docs/release/dependency-security-audit.md` and remains release-blocking: the libp2p 0.56 and rumqttc-next slices removed the old `ring 0.16`, `rustls-webpki 0.101.7`, and MQTT `rustls-webpki 0.102.8` findings, but vulnerable `hickory-proto 0.25.2` and `libcrux-chacha20poly1305 0.0.7` remain.
