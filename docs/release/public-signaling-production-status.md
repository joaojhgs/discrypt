...
- The Tauri command `handle_text_control_frame` accepts typed frames from the attached peer DataChannel. Envelope frames are verified through `receive_text_delivery_envelope` and return a receipt response frame; receipt frames are route-bound to the live text/control transport peer before they update the sender timeline. A two-state-file regression proves Bob's received-envelope/receipt row and Alice's final `peer_receipt` survive disk reload. The backend-owned `TextControlDataTransport` runtime and periodic pump list pending outbox frames, send them over the direct peer transport, record the hash-guarded handoff, receive response frames, verify/apply signed receipts, emit metrics, and persist final receipt state. The pump fails closed when no runtime is attached, no text session is active, or the runtime session does not match the active session; these cases surface typed command errors instead of silently marking delivery.
- Signed `TextDeliveryReceipt` application is no longer exposed as a caller-controlled UI command. The backend derives the admitted DM/group recipient verifying key from stored peer identity, checks the immutable expected route set, and requires the observed transport route to match before marking the message as `peer_receipt`.
- Tampered receipts are rejected with `receipt_verification_failed` and do not upgrade the message state.
- The UI command surface has typed envelope/receipt/text-control-frame/receipt-view models plus native-only `receiveTextDeliveryEnvelope(...)` and `handleTextControlFrame(...)` bindings; browser fallback stays honest and reports that signed envelope/receipt verification requires the Rust/Tauri backend.
- This is the signed state-transition boundary needed for remote delivery honesty. A two-profile backend test proves a distinct Bob profile identity can verify Alice's envelope, persist a received-envelope row, sign a receipt, and return it for Alice to verify. Environment-gated public MQTT and Nostr tests carry Alice's serialized text/control frame to Bob's transport answerer and return Bob's signed receipt before Alice applies `peer_receipt`. The strict two-profile Tauri harness additionally requires live direct DataChannels, persisted receipt state, native voice frames sent and received in both directions, and playback queues drained by both UIs. Public cross-region reachability remains an environment-dependent release check rather than a property the local deterministic suite can guarantee.
