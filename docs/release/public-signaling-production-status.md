...
- Latest live Tauri MQTT receipt proof passed with `DISCRYPT_DESKTOP_PUBLIC_MQTT_RECEIPT_E2E=1` against `mqtts://broker.emqx.io:8883`; the harness has now been tightened so the response frame is generated by Bob's `handle_text_control_frame` receiver path and applied by Alice's receipt-frame handler. It verifies both DataChannel directions and signed receipt semantics, but still does not provide a persistent runtime session pump or two installed app processes.
