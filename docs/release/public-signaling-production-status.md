...
- **Full app-level two-Tauri-instance DM/group text + voice E2E over those adapters: not done.** Current proof reaches the Rust transport signaling adapter layer, sealed WebRTC offer/answer exchange, and a real WebRTC DataChannel frame over public MQTT/Nostr rendezvous; it is still not the complete Tauri UI/backend/media-plane proof.
...
- [ ] Establish the same data-channel path across two real Tauri app profiles/devices from UI-driven DM/group state.
