...
- [ ] Complete IPFS/libp2p production hardening:
  ...
  - typed IPFS health now covers `topic_mesh_unavailable` and unreachable `bootstrap_connect` failures as `provider_unhealthy` plus oversized envelopes as `provider_message_too_large`, while invalid/duplicate/overflow profile rejection remains policy-level; remaining typed-health gaps are duplicate storms and provider-unhealthy swarm events,
