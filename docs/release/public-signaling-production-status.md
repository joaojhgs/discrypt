...
These are real public signaling proofs at the provider adapter boundary. A new optional public-provider WebRTC data-channel test exists, but latest MQTT/Nostr runs still fail while applying the remote answer after public-provider latency (`set remote answer failed: Disconnected(WriteNotify)`), so full data-channel delivery remains open.
