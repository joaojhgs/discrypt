...
These are real public signaling proofs at the provider adapter boundary. A new optional public-provider WebRTC data-channel test exists, but latest MQTT/Nostr runs using public Google STUN still fail while applying the remote answer after public-provider latency (`set remote answer failed: Disconnected(WriteNotify)`), so full data-channel delivery remains open.
...
- [ ] Use the signaling adapters to exchange real WebRTC offer/answer/candidate payloads generated by the app. Optional public MQTT/Nostr WebRTC data-channel tests now exist, but latest live runs fail at remote-answer application after provider latency.
...
| Optional public provider-signaled WebRTC data-channel proof | `DISCRYPT_PUBLIC_MQTT_WEBRTC_E2E=1 ... public_mqtt_signals_real_webrtc_datachannel_roundtrip` and `DISCRYPT_PUBLIC_NOSTR_WEBRTC_E2E=1 ... public_nostr_signals_real_webrtc_datachannel_roundtrip` | Test exists and is skipped by default; latest live MQTT and Nostr runs use `stun:stun.l.google.com:19302`, exchange offer/answer through the provider, but fail before DataChannel open with `set remote answer failed: Disconnected(WriteNotify)`. |
