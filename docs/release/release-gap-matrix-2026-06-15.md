...
| Encrypted peer-assisted overlay relay | planned | Peer-overlay routing is a separate application route, not a compatibility leg in the direct-ICE/configured-TURN planner. | Overlay route evidence, relay authority, ciphertext-only validation, and fail-closed behavior are not production-proven. |
