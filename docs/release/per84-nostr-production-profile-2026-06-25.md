...
- `RUSTUP_TOOLCHAIN=1.89.0 cargo test -p discrypt-transport --features nostr-adapter nostr_ -- --test-threads=1`
  - Included local/unit Nostr coverage for multi-relay preservation/dedupe, loopback `ws://127.0.0.1` validation, public cleartext relay rejection, privacy-safe event content/tag shape, typed oversized-envelope failure, structured provider failure mapping, and existing Nostr public-signaling/WebRTC test modules with skip-gated tests not asserted as live evidence.
...
The repository does not currently include an embedded local Nostr relay runner or fixture. Local relay coverage in this branch is therefore validation-level evidence for loopback `ws://127.0.0.1...` profiles, not a live local relay roundtrip. The live relay roundtrip and multi-relay fallback evidence above are public Nostr relay evidence.
