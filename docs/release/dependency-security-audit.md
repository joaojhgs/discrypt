...
```bash
cargo audit --json > /tmp/discrypt-cargo-audit-rumqttc-next.json
```

Result: **not release-clean**. After the libp2p 0.56 and MQTT client remediation slices, `cargo audit` still exits non-zero with **3 vulnerability hits**, plus unmaintained and unsound warnings.
...
## Verification evidence for the libp2p 0.56 / rumqttc-next remediation slices
...
Latest audit result after these slices: **3 vulnerability hits remain** (`hickory-proto 0.25.2` x2 and `libcrux-chacha20poly1305 0.0.7` x1).
