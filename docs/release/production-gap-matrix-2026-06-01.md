...
- `docs/release/g011-production-readiness-matrix.md` — current G011 readiness and G012 boundary reconciliation.
...
| Inspector/debug rails and local dev banners | Diagnostic escape hatch | Manual peer/role attach controls were removed; diagnostics must not let users supply runtime peer IDs. |
...
| G010 Release harness and automation | Partial | Adapter/public matrix automation is wired through `npm --prefix apps/ui run test:g010-adapter-public-matrix` with deterministic local STUN/TURN gates and explicit env-gated MQTT/Nostr/IPFS/QUIC/TURN skips; real Tauri two-user harness work remains separate. | Reproducible script command and archived artifacts; no G011/G012 final production claim. |
| G011 Production ready | In progress / not closed | All prior production gates clean on the integrated leader branch; Linux packaging/build docs current; unsupported paths disabled or honest; release matrix and final quality evidence reconciled in `docs/release/g011-production-readiness-matrix.md`; not complete until final G011 quality evidence lands and the leader-owned checkpoint records it. | Final quality gate, code review, build/package verification, static release docs gates, and leader-owned checkpoint evidence. |
...
- G011 and G012 stay separate: G011 can close only after integrated production-readiness gates pass and final evidence is retained; G012 still requires the real two-installed-user Tauri text-plus-voice artifact bundle. G012 is not complete until real Tauri and two-user artifacts exist.
