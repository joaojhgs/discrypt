...
const fallbackSnapshot: AppSnapshot = {
  ...
  servers: [
    {
      ...
      channels: [
        ...
        { name: 'Voice Lobby', kind: 'Voice', retention_status: 'Session-state only; media-frame E2E gate required before production voice claims' },
      ],
    },
  ],
  invite: {
  ...
  voice: {
    ...
    relay_copy: 'Relays see SFrame ciphertext only in harness gates; production media waits for real audio-frame E2E',
    android_path: 'Android uses encoded transforms when available, otherwise the native webrtc-rs contingency',
  },
  voice_session: {
    joined: true,
    participants: [
      { id: 'alice', name: 'Alice', role: 'you', speaking: true, muted: false, volume: 82 },
      { id: 'bob', name: 'Bob', role: 'friend', speaking: true, muted: false, volume: 68 },
      { id: 'ops', name: 'Ops relay', role: 'route', speaking: false, muted: true, volume: 38 },
    ],
    status_copy: 'Voice session state is command-backed; real audio-frame transport remains release-gated',
  ...
  messages: [
    {
      ...
      state: 'plaintext allowed by current local retention cache; encrypted envelope facade recorded by harness',
    },
    {
      id: 'locked-msg-1',
      channel: '#general',
...
function invokeOrFallback<T>(command: string, args: Record<string, unknown> | undefined, fallback: () => T): Promise<T> {
...
export async function loadAppSnapshot(): Promise<AppSnapshot> {
  return invokeOrFallback<AppSnapshot>('app_snapshot', undefined, () => fallbackSnapshot);
}

export async function verifySafetyNumber(request: SafetyVerificationRequest): Promise<SafetyVerificationResult> {
  return invokeOrFallback<SafetyVerificationResult>('verify_safety_number', { request }, () => {
...
export async function createGroup(request: CreateGroupRequest): Promise<AppSnapshot> {
  return invokeOrFallback<AppSnapshot>('create_group', { request }, () => {
    fallbackSnapshot.servers.unshift({
      name: request.name || 'private lab',
      role: 'owner',
      channels: [
        { name: '#general', kind: 'Text', retention_status: `${request.retention || fallbackSnapshot.retention.selected}; older messages lock, not vanish` },
        { name: 'Voice Lobby', kind: 'Voice', retention_status: 'Session-state only; media-frame E2E gate required before production voice claims' },
      ],
    });
...
export async function joinGroup(request: JoinGroupRequest): Promise<AppSnapshot> {
  return invokeOrFallback<AppSnapshot>('join_group', { request }, () => {
    fallbackSnapshot.servers.unshift({
      name: request.invite_code.includes('enclave') ? 'joined enclave' : 'joined group',
      role: 'member',
      channels: [
        { name: '#general', kind: 'Text', retention_status: `${fallbackSnapshot.retention.selected}; older messages lock, not vanish` },
        { name: 'Voice Lobby', kind: 'Voice', retention_status: 'Session-state only; media-frame E2E gate required before production voice claims' },
      ],
    });
    return fallbackSnapshot;
...
export async function createChannel(request: CreateChannelRequest): Promise<AppSnapshot> {
  return invokeOrFallback<AppSnapshot>('create_channel', { request }, () => {
    const server = fallbackSnapshot.servers.find((item) => item.name === request.server_name) ?? fallbackSnapshot.servers[0];
    const name = request.kind === 'Text' ? `#${request.name.replace(/^#/, '') || 'secure-room'}` : request.name || 'Voice Lobby';
    if (!server.channels.some((channel) => channel.name === name)) {
      server.channels.push({ name, kind: request.kind, retention_status: `${fallbackSnapshot.retention.selected}; older messages lock, not vanish` });
    }
    return fallbackSnapshot;
  });
}

export async function savePreferences(request: SavePreferencesRequest): Promise<AppSnapshot> {
...
export async function setSelfMute(request: SelfMuteRequest): Promise<AppSnapshot> {
  return invokeOrFallback<AppSnapshot>('set_self_mute', { request }, () => {
    fallbackSnapshot.voice_session.participants = fallbackSnapshot.voice_session.participants.map((participant) =>
      participant.id === 'alice' ? { ...participant, muted: request.muted, speaking: fallbackSnapshot.voice_session.joined && !request.muted } : participant,
    );
    return fallbackSnapshot;
  });
}

export async function setSpeakerVolume(request: SpeakerVolumeRequest): Promise<AppSnapshot> {
  return invokeOrFallback<AppSnapshot>('set_speaker_volume', { request }, () => {
    fallbackSnapshot.voice_session.participants = fallbackSnapshot.voice_session.participants.map((participant) =>
      participant.id === request.participant_id ? { ...participant, volume: request.volume } : participant,
    );
    return fallbackSnapshot;
  });
}

export async function sendMessage(request: SendMessageRequest): Promise<AppSnapshot> {
