...
use discrypt_mls_delivery::{
    decrypt_text_envelope, DeliveryError, InMemoryTextAuthorLog, InMemoryTextReceiveEvents,
    InMemoryTextRecipientStore, InMemoryTextSendEvents, InMemoryTextTransport, TextDeliveryReceipt,
    TextDeliveryReceiptInput, TextInboundPipeline, TextInboundRequest, TextMessageEnvelope,
    TextMessageEnvelopeInput, TextOutboundPipeline, TextOutboundRequest, TextReceiveState,
    TextRenderState, TextRetentionMetadata, TextSelectedRoute,
};
...
pub enum TextControlFrameView {
    /// Joiner request carrying an OpenMLS key package generated by its local provider.
    ...
    OpenMlsAdmissionWelcome {
        ...
        /// Owner group epoch after add-member commit.
        epoch: u64,
...
/// Protected native Rust media frame safe to carry inside sealed voice signaling.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct NativeVoiceProtectedFrameView {
    /// SFrame KID copied from Rust sender binding; not a secret.
    pub kid: Vec<u8>,
    /// Sender counter copied from Rust sender state; not a secret.
    pub counter: u64,
    /// Authenticated ciphertext bytes from Rust Opus/SFrame.
    pub bytes: Vec<u8>,
}
...
pub struct NativeVoiceMediaSignalPayload {
    ...
    /// Voice channel id.
    pub channel_id: String,
    /// Provider/runtime peer id that produced the frame.
    pub from_peer_id: String,
    /// Provider/runtime peer id expected to receive the frame.
    pub to_peer_id: String,
    /// Production-facing native media path label.
    pub media_path: String,
    /// Rust media boundary used to generate the protected frame.
    pub boundary: String,
    /// Deterministic native capture source used by automated proof runs.
    pub capture_source: String,
    /// Local audio-level evidence computed from the generated Rust PCM frame.
    pub rms_i16: u16,
    /// Peak sample evidence computed from the generated Rust PCM frame.
    pub peak_i16: u16,
    /// Whether Rust VAD marked the generated frame as speaking.
    pub speaking: bool,
    /// Number of Opus frames generated by Rust.
    pub opus_frames: u16,
    /// Number of protected media frames generated by Rust SFrame.
    pub protected_frames_count: u16,
    /// Total Opus payload bytes before SFrame protection.
...
pub fn join_group(request: JoinGroupRequest) -> AppStateView {
    mutate_app_service(|state| {
        ...
        if let Some(group_id) = state
            ...
            .and_then(|context| context.group_id.clone())
        {
            if let Err(error) = state.queue_openmls_admission_key_package(&group_id) {
                state.push_command_error(
                    "mls.admission_request_failed",
                    "join_group",
                    "openmls_admission_key_package_failed",
                    error,
                    "Retry joining when the OpenMLS provider can create a member key package",
                );
                return;
            }
        }
...
impl PersistedAppState {
    ...
    fn queue_openmls_admission_key_package(&mut self, group_id: &str) -> Result<(), String> {
        ...
        let member_identity = self.local_user_id();
        let engine = OpenMlsGroupEngine::open(app_openmls_store_path())
            .map_err(|error| format!("OpenMLS joiner provider could not be opened: {error}"))?;
        let package = engine
            .generate_member_package(member_identity.as_bytes())
            .map_err(|error| format!("OpenMLS key package could not be generated: {error}"))?;
        ...
        let message_id = stable_id("openmls-admission-request", group_id, self.next_sequence);
        if !self
            .text_control_outbox
            .iter()
        ...
        self.push_event(
            "mls.admission_key_package_queued",
            format!(
                "Queued OpenMLS admission key package for {}",
                redacted_observable_ref("group", group_id)
            ),
        );
        Ok(())
    ...
    fn openmls_admission_welcome_from_frame(
        ...
        let owner_record = self
            ...
            .cloned()
            .ok_or_else(|| format!("OpenMLS owner group handle for {group_id} is not persisted"))?;
        let owner_signer_public_key = hex::decode(&owner_record.signer_public_key_hex)
            .map_err(|error| format!("OpenMLS owner signer handle is not hex: {error}"))?;
        let member_signer_public_key = hex::decode(signer_public_key_hex)
            .map_err(|error| format!("OpenMLS member signer handle is not hex: {error}"))?;
        let package = OpenMlsMemberPackage::from_key_package_bytes(
            member_signer_public_key,
            key_package_bytes,
        )
        .map_err(|error| format!("OpenMLS key package could not be decoded: {error}"))?;
        let store_path = owner_record
            .openmls_store_path
            .as_ref()
            .map(PathBuf::from)
            .unwrap_or_else(app_openmls_store_path);
        ...
        engine
            ...
            .map_err(|error| format!("OpenMLS owner group could not be loaded: {error}"))?;
        let result = engine
            .add_member_package(group_id, &package)
            .map_err(|error| format!("OpenMLS member package could not be added: {error}"))?;
        let welcome_bytes = result.welcome.clone().ok_or_else(|| {
            "OpenMLS add_member_package did not produce a Welcome for the joiner".to_owned()
        })?;
    ...
    fn join_openmls_from_admission_welcome_frame(
        ...
        let signer_public_key = hex::decode(member_signer_public_key_hex)
            .map_err(|error| format!("OpenMLS member signer handle is not hex: {error}"))?;
        let mut engine = OpenMlsGroupEngine::open(app_openmls_store_path())
            .map_err(|error| format!("OpenMLS joiner provider could not be opened: {error}"))?;
        let snapshot = engine
            .join_from_welcome(group_id, &signer_public_key, welcome_bytes)
            .map_err(|error| format!("OpenMLS Welcome could not be joined: {error}"))?;
        let mut confirmation_hash = Sha256::new();
        confirmation_hash.update(&snapshot.confirmation_tag);
        ...
        self.push_event(
            "mls.admission_welcome_joined",
            format!(
                "Joined OpenMLS group from admission Welcome for {}",
                redacted_observable_ref("group", group_id)
            ),
        );
        let retried = self.retry_openmls_plaintext_render_for_group(group_id);
        if retried > 0 {
            self.push_event(
    ...
    fn retry_openmls_plaintext_render_for_group(&mut self, group_id: &str) -> usize {
        let delivery_prefix = format!("group:{group_id}:channel:");
        let candidates = self
            .text_delivery_envelopes
            .iter()
            .filter(|record| record.group_id.starts_with(&delivery_prefix))
            .cloned()
            .collect::<Vec<_>>();
        ...
        for record in candidates {
            let Some(message_index) = self.messages.iter().position(|message| {
                message.message_id == record.message_id
                    && message.target.kind == "channel"
                    && message.target.group_id.as_deref() == Some(group_id)
                    && message.state_key != "received_plaintext"
            }) else {
                continue;
            };
            let target = self.messages[message_index].target.clone();
            let Ok(sender_key) = verifying_key_from_hex(&record.sender_verifying_key_hex)
            ...
            if record
                .envelope
                .verify(&record.group_id, &sender_key)
                .is_err()
            {
                continue;
            }
            let Ok((text_exporter_secret, _current_epoch)) =
                self.openmls_text_exporter_for_receive(&target, &record.group_id)
            else {
                continue;
            };
            let Ok(plaintext) =
                decrypt_text_envelope(&record.group_id, &text_exporter_secret, &record.envelope)
            else {
                continue;
            };
            let message = &mut self.messages[message_index];
            message.body = String::from_utf8_lossy(&plaintext).into_owned();
            message.status = "signed encrypted peer envelope verified and decrypted after OpenMLS admission completed".to_owned();
            message.state_key = "received_plaintext".to_owned();
            message.state_label = "Plaintext received".to_owned();
            message.state_detail = format!(
                "Verified sender={} {} {}; plaintext rendered after delayed OpenMLS Welcome admission",
                key_fingerprint(&sender_key),
                redacted_message_ref(&record.message_id),
                redacted_observable_ref("group_binding", &record.group_id)
            );
    ...
    fn openmls_text_exporter_secret(&self, group_id: &str) -> Result<Vec<u8>, String> {
        let (secret, _epoch) = self.openmls_exporter_secret(
            ...
            TEXT_EXPORTER_LABEL,
            group_id.as_bytes(),
            "OpenMLS text exporter",
        )?;
        Ok(secret)
    }

    fn openmls_media_exporter_secret(
        &self,
        group_id: &str,
        channel_id: &str,
        session_id: &str,
        ...
        context.extend_from_slice(b"discrypt-tauri-voice-v1");
        context.extend_from_slice(&(group_id.len() as u64).to_be_bytes());
        context.extend_from_slice(group_id.as_bytes());
        context.extend_from_slice(&(channel_id.len() as u64).to_be_bytes());
        context.extend_from_slice(channel_id.as_bytes());
        context.extend_from_slice(&(session_id.len() as u64).to_be_bytes());
        context.extend_from_slice(session_id.as_bytes());
        self.openmls_exporter_secret(
            group_id,
            "discrypt/media",
            &context,
            "OpenMLS media exporter",
        )
    }

    fn openmls_exporter_secret(
        &self,
        group_id: &str,
        label: &str,
        context: &[u8],
        ...
        let snapshot = engine
            ...
            .map_err(|error| format!("OpenMLS group could not be loaded: {error}"))?;
        let secret = engine
            .export_secret(group_id, label, context, 32)
        ...
        T: discrypt_transport::TextControlDataTransport + ?Sized,
    {
        ...
        for frame in &pending {
            ...
            let expects_text_receipt = matches!(frame.frame, TextControlFrameView::Envelope { .. });
            ...
            TextControlFrameView::OpenMlsAdmissionWelcome {
                ...
                if let Err(error) = self.join_openmls_from_admission_welcome_frame(
                    ...
                    self.push_command_error(
                        ...
                        error,
                        "Only install an OpenMLS Welcome whose group id and confirmation tag match",
                    );
                }
                None
            }
...
mod tests {
    ...
    fn g012_two_profile_group_text_delivery_bidirectional_persists() -> Result<(), String> {
        ...
        assert!(
            bob_pending_admission.frames.iter().any(|frame| matches!(
                frame.frame,
                TextControlFrameView::OpenMlsAdmissionKeyPackage { .. }
            )),
            "joining through an invite must queue a real OpenMLS admission key package"
        );
        ...
        let admission_session = start_text_session(StartTextSessionRequest {
            scope_label: Some("g012-bob-openmls-admission".to_owned()),
            data_channel_probe: false,
            adapter_kind: None,
        });
        assert!(
        ...
        assert!(
            alice_after_admission
                .openmls_groups
                .iter()
                .any(|handle| handle.group_id == alice_group_id && handle.epoch > 0),
            "owner must persist the rotated OpenMLS epoch after admitting Bob"
        );
        let owner_handle = alice_after_admission
            .openmls_groups
            .iter()
            .find(|handle| handle.group_id == alice_group_id)
            .cloned()
            .ok_or_else(|| "owner OpenMLS handle missing after admission".to_owned())?;
        let joiner_handle = bob_after_admission
            .openmls_groups
            .iter()
            .find(|handle| handle.group_id == alice_group_id)
            .cloned()
            .ok_or_else(|| "joiner OpenMLS handle missing after admission".to_owned())?;
        let owner_signer = hex::decode(&owner_handle.signer_public_key_hex)
            .map_err(|error| format!("owner signer hex failed: {error}"))?;
        let joiner_signer = hex::decode(&joiner_handle.signer_public_key_hex)
            .map_err(|error| format!("joiner signer hex failed: {error}"))?;
        let mut owner_engine =
            OpenMlsGroupEngine::open(openmls_store_path_for_app_state_path(&alice_path))
                .map_err(|error| format!("owner OpenMLS provider open failed: {error}"))?;
        let mut joiner_engine =
            OpenMlsGroupEngine::open(openmls_store_path_for_app_state_path(&bob_path))
                .map_err(|error| format!("joiner OpenMLS provider open failed: {error}"))?;
        owner_engine
            .load_group(&alice_group_id, &owner_signer)
            .map_err(|error| format!("owner OpenMLS load failed: {error}"))?;
        joiner_engine
            .load_group(&alice_group_id, &joiner_signer)
            .map_err(|error| format!("joiner OpenMLS load failed: {error}"))?;
        let owner_exporter = owner_engine
            .export_secret(
                &alice_group_id,
                "discrypt/text",
                b"g012-openmls-admission-control-frame",
                32,
            )
        ...
        let owner_exporter_sha256 = hex::encode(Sha256::digest(&owner_exporter));
        let joiner_exporter_sha256 = hex::encode(Sha256::digest(&joiner_exporter));
